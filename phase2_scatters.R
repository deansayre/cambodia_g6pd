...
phase2 <- import("phase2_clean.RDS")%>% 
  mutate(hem_class = case_when(age <12 & hb_ven_heman >= 11.5 ~ 0, 
                               age <12 & hb_ven_heman >= 11.0 ~ 1,
                               age <12 & hb_ven_heman >= 8.0 ~ 2,
                               age <12 & hb_ven_heman < 8.0 ~ 3,
                               age <15 & hb_ven_heman >= 12.0 ~ 0, 
                               age <15 & hb_ven_heman >= 11.0 ~ 1,
                               age <15 & hb_ven_heman >= 8.0 ~ 2,
                               age <15 & hb_ven_heman < 8.0 ~ 3,
                               sex == 1 & hb_ven_heman >= 13.0 ~ 0, 
                               sex == 1 & hb_ven_heman >= 11.0 ~ 1,
                               sex == 1 & hb_ven_heman >= 8.0 ~ 2,
                               sex == 1 & hb_ven_heman < 8.0 ~ 3,
                               pregnancy == 1 & hb_ven_heman >= 11.0 ~ 0, 
                               pregnancy == 1 & hb_ven_heman >= 10.0 ~ 1,
                               pregnancy == 1 &  hb_ven_heman >= 7.0 ~ 2,
                               pregnancy == 1 &  hb_ven_heman < 7.0 ~ 3,
                               pregnancy == 2 & hb_ven_heman >= 12.0 ~ 0, 
                               pregnancy == 2 & hb_ven_heman >= 11.0 ~ 1,
                               pregnancy == 2 &  hb_ven_heman >= 8.0 ~ 2,
                               pregnancy == 2 &  hb_ven_heman < 8.0 ~ 3,
                               TRUE ~ NA_real_))
...
g6pd_codes <- import("g6pd_codes1.xlsx")  # a dictionary for the facade() function

genotype <- import("g6pd_genotype.xlsx") %>% 
  linelist::clean_data(guess_dates = F)%>% 
  mutate(row = row_number()) %>% 
  pivot_longer(cols =! row,
               names_to = "type", 
               values_to = "value") %>% 
  mutate(genotype = word(type, 1, sep = "_"), 
         assay = word(type, 2, sep = "_")) %>% 
  drop_na(value) %>% 
  group_by(row, genotype) %>% 
  mutate(id = cur_group_id()) %>% 
  ungroup() %>% 
  select(-c(row, type)) %>% 
  pivot_wider(names_from = assay)

samples_genotyped <- import("samples_g6pd_sequenced.xlsx")%>% 
  linelist::clean_data(guess_dates = F)
